...
`
	...
	if records[0].Tokens.Input != 60 || records[0].Tokens.Cached != 40 {
		t.Fatalf("first total usage not parsed correctly: %+v", records[0].Tokens)
	}
	if records[1].Tokens.Input != 30 || records[1].Tokens.Output != 10 || records[1].Tokens.Cached != 20 || records[1].Tokens.Reasoning != 2 {
		t.Fatalf("total delta not parsed correctly: %+v", records[1].Tokens)
	}
	if records[2].Tokens.Input != 15 || records[2].Tokens.Output != 8 || records[2].Tokens.Cached != 5 || records[2].Tokens.Reasoning != 3 {
